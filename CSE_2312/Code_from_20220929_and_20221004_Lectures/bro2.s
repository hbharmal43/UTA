...
// uint32_t binaryToUint32(const char str[])
// address of string in R0, return value in R0
binaryToUint32:
    MOV R1, R0         // R1 = &str[0]
    MOV R0, #0         // R0 = value = 0
btu_loop:
    ...
    CMP R2, #0         // is R2 = null?
    BEQ btu_end        // yes, exit loop
    CMP R2, #'1'       // is R2 = '1'?
    MOV R0, R0, LSL #1 // shift value to left
    ORREQ R0, R0, #1   // yes, set bit 0
    B btu_loop         // get next character
btu_end:
    BX LR
